...
def update_statistics(stats_request: schemas.StatisticsUpdateRequest, db: Session = Depends(get_db)):
    doc_id = stats_request.document_id
    
    # 1. Update or Create AnnotationSummary
    summary = db.query(models.AnnotationSummary).filter(models.AnnotationSummary.document_id == doc_id).first()
    if not summary:
...
def get_global_summary(db: Session = Depends(get_db)):
    # Calculate global totals across all documents
    summaries = db.query(models.AnnotationSummary).all()
...
def get_page_corrections(db: Session = Depends(get_db)):
    pages = db.query(models.PageCorrection).order_by(models.PageCorrection.id.desc()).all()
    return pages

@router.get("/statistics/logs", response_model=List[schemas.AnnotationLogResponse])
def get_annotation_logs(db: Session = Depends(get_db)):
    logs = db.query(models.AnnotationLog).order_by(models.AnnotationLog.log_id.desc()).limit(100).all()
    return logs
